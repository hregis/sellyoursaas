...
if [[ "$mode" == "changephpversion" ]]; then
	if [[ "x$phpfpm" == "x" ]]; then
		...
		# Handles both an instance already on php-fpm (switching version) and one still on
		# mod_php (first-time migration, no existing SetHandler to replace) - see
		# switch_instance_phpversion.sh for details.
