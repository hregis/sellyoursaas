...
class SellYourSaasUtils
{
	...
	/**
	 *  Send reminders by emails to customers after registration.
	 *  CAN BE A CRON TASK
	 *
	...
	public function sendEmailsRemindersAfterRegistration($nbdays = 0, $template = '', $forcerecipient = '')
	{
		...
		if (!isModEnabled('sellyoursaas')) {	// Should not happen. If module disabled, cron job should not be visible.
			$this->output .= $langs->trans('ModuleNotEnabled', $langs->transnoentitiesnoconv("SellYourSaas"));
			return 0;
		}
		...
		$tmpcompany = new Societe($this->db);
		...
		$this->output .= 'We search thirdparties with sql='.$sql;
		$this->output .= '<br>';
		...
		if ($resql) {
			while ($obj = $this->db->fetch_object($resql)) {
				...
				if ($res > 0) {
					...
					if ($tmpcompany->default_lang) {
						$outputlangs->setDefaultLang($tmpcompany->default_lang);
					...
					$substitutionarray = getCommonSubstitutionArray($outputlangs, 0, null, $tmpcompany);

					complete_substitutions_array($substitutionarray, $outputlangs, $tmpcompany);
					...
					if (!$loopError && !empty($to)) {
						...
						if ($resultsendmail) {
							...
							$actioncomm->socid = $tmpcompany->id; // To link to a company
							$actioncomm->contact_id = 0;

							$actioncomm->code = 'AC_EMAIL';
							$actioncomm->label = 'sendEmailsRemindersAfterRegistration (nbdays='.$nbdays.' template='.$template.' forcerecipient='.$forcerecipient.')';
							$actioncomm->note_private = $sendContent;
							$actioncomm->fk_project = $tmpcompany->fk_project;
							...
							$actioncomm->elementtype = 'societe';
							$actioncomm->elementid = $tmpcompany->id;
	...
	public function doAlertSoftEndTrial()
	{
		...
		$delayindaysshort = getDolGlobalString('SELLYOURSAAS_NBDAYS_BEFORE_TRIAL_END_FOR_SOFT_ALERT');
		$delayindayshard = getDolGlobalString('SELLYOURSAAS_NBDAYS_BEFORE_TRIAL_END_FOR_HARD_ALERT');
					...
					if ($obj) {
						...
						if ($result > 0) {
							if ($invoice->status == Facture::STATUS_DRAFT) {
								...
								// Check amount
								if (getDolGlobalString('SELLYOURSAAS_MAX_MONTHLY_AMOUNT_OF_INVOICE')) {
									$amountofinvoice = $invoice->total_ht;
									$monthfactor = 1;
									if ($invoice->fk_fac_rec_source > 0) {
										include_once DOL_DOCUMENT_ROOT.'/compta/facture/class/facture-rec.class.php';
		...
		foreach ($invoices as $invoice) {
			...
			if ($amountstripe > 0) {
				try {
					...
					$nbhoursbetweentries    = (!getDolGlobalString('SELLYOURSAAS_NBHOURSBETWEENTRIES') ? 49 : $conf->global->SELLYOURSAAS_NBHOURSBETWEENTRIES);				// Must have more that 48 hours + 1 between each try (so 1 try every 3 daily batch)
					$nbdaysbeforeendoftries = (!getDolGlobalString('SELLYOURSAAS_NBDAYSBEFOREENDOFTRIES') ? 35 : $conf->global->SELLYOURSAAS_NBDAYSBEFOREENDOFTRIES);
					$labeltouse = '';
							...
							if ($stripecard) {  // Can be card_... (old mode) or pm_... (new mode)
								...
								if (preg_match('/^card_/', $stripecard->id)) { // Using old method
									...
									if (!empty($paymentintent->status) && ($paymentintent->status === 'succeeded' || $paymentintent->status === 'processing')) {
										...
										dol_syslog("error creation paymentintent = ".var_export($paymentintent, true), LOG_DEBUG);
	...
	private function doSuspendInstances($mode, $noapachereload = 0, $maxnbofinstances = 0)
	{
		...
		$gracedelay=9999999;
		if ($mode == 'test') {
			$gracedelay=getDolGlobalString('SELLYOURSAAS_NBDAYS_AFTER_EXPIRATION_BEFORE_TRIAL_SUSPEND');
		}
		if ($mode == 'paid') {
			$gracedelay=getDolGlobalString('SELLYOURSAAS_NBDAYS_AFTER_EXPIRATION_BEFORE_PAID_SUSPEND');
		}
		if ($gracedelay < 1) {
	...
	public function doUndeployOldSuspendedInstances($mode)
	{
		...
		$MAXPERCALL = (!getDolGlobalString('SELLYOURSAAS_MAX_UNDEPLOY_PER_CALL') ? 5 : $conf->global->SELLYOURSAAS_MAX_UNDEPLOY_PER_CALL);       // Undeploy can be long (1mn). So we limit to 5 per call
	...
	public function sellyoursaasRemoteAction($remoteaction, $object, $appusername = 'admin', $email = '', $password = '', $forceaddevent = '0', $comment = '', $timeout = 90, $newip = '')
	{
				...
				*/
				...
				if ($connection) {
					...
					if (function_exists('ssh2_disconnect')) {
						if (!getDolGlobalString('SELLYOURSAAS_SSH2_DISCONNECT_DISABLED')) {
		...
		foreach ($listoflinesqualified as $tmparrayoflinesqualified) {
			...
			if ($doremoteaction == 1) {
				...
				$automigrationtmpdir = $dirfortmpfiles."/automigration".((!empty($object->socid) && $object->socid > 0) ? "_".$object->socid : "").".tmp";
		...
		*/
		...
		if (getDolGlobalString('SELLYOURSAAS_DATADOG_ENABLED') && $remoteaction != 'backup') {
			try {
				dol_include_once('/sellyoursaas/core/includes/php-datadogstatsd/src/DogStatsd.php');
	...
	public function getRemoteServerDeploymentIp($domainname, $onlyifopen = 0)
	{
		...
		if (!getDolGlobalString('SELLYOURSAAS_OBJECT_DEPLOYMENT_SERVER_MIGRATION')) {
			...
			if (! $found) {
				...
				$tmparray = explode(',', getDolGlobalString('SELLYOURSAAS_SUB_DOMAIN_IP'));
